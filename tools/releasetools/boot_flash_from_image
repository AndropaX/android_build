...
if path.exists(defconfig):
    ...
    if prekernel_version:
        ...
        kernel_version = "%s-%s" % (kernel_version or "RR", device)
    else:
        kernel_version = "RR-%s-kernel" % device
else:
    kernel_version = "RR-%s-kernel" % device

updater = "%s/obj/EXECUTABLES/updater_intermediates/updater" % out
updater = "%s/obj_arm/EXECUTABLES/updater_intermediates/updater" % out
...
# create the contents
contents = "ui_print(\"installing Resurrected Kernel...\");\n"
contents += "ui_print(\"by AndropaX\");\n"
